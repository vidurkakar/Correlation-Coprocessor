...
typedef unsigned              index_t;
	...
	int fdi;
	void *virtual_base;
	volatile unsigned long *corr_base;
...
void  hw_bulkcorrelate(sample_hptr sig,
	...
	int  mac1, mac2, mac3, mac4, max = 0;
	unsigned i, j, k, maxindex = 0;
	...
	for(i = 0; i < 1024; i++) {
		for(j = 0; j < 128; j = j + 4) {
			corr_base[4] = *(int *)(sig + k + j);
			corr_base[5] = *(int *)(sig + k + j + 4);
			corr_base[6] = *(int *)(sig + k + j + 8);
			corr_base[7] = *(int *)(sig + k + j + 12);
			corr_base[8] = *(int *)(sig + k + j + 16);
			
			corr_base[0xA] = 1;
			while(corr_base[0xB] == 0);
		
			corr_base[0xA] = 0;
			while(corr_base[0xB] == 1);
...
int main() {
  ...
  if( ( fdi = open( "/dev/mem", ( O_RDWR | O_SYNC ) ) ) == -1 ) {
		printf( "ERROR: could not open \"/dev/mem\"...\n" );
		//return( 1 );
	} 
	virtual_base = mmap( NULL, HW_REGS_SPAN, ( PROT_READ | PROT_WRITE ), MAP_SHARED,  fdi,  HW_REGS_BASE );	
	corr_base = virtual_base +  ( (unsigned long)( ALT_LWFPGASLVS_OFST + HW_CORRELATE_0_BASE) & ( unsigned long)( HW_REGS_MASK ) );
